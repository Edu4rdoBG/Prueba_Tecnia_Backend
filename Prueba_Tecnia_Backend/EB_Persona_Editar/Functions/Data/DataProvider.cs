...
namespace EB_Persona_Editar.Functions.Data
{
    public static class DataProvider
    {
        ...
        public static bool Execute_sp_update_eb_persona(RequestPersona input, out string mensaje, out PersonaModel persona)
        {
            ...
            try
            {
                ...
                LambdaLogger.Log(string.Concat("call sp_update_eb_persona()", DateTime.Now));

                MySqlCommand cmd = new("sp_update_eb_persona", connection);
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (fila > 0)
                    {
                        mensaje = $"Se realizo el update con exito, fila afectada: {fila} {DateTime.Now}";
                    ...
                    {
                        mensaje = $"No se logro realizar el update, fila afectada: {fila} {DateTime.Now}";
                        LambdaLogger.Log(mensaje);
                        return false;
                    }                    
                }
